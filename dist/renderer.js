...
const tracker_1 = require("./widget/tracker");
document.addEventListener('DOMContentLoaded', () => {
    const studyTrackerWidget = new widget_1.Widget();
    const studyTracker = new tracker_1.Tracker();
    ...
    function updateDisplay() {
        ...
        if (currentSessionElement && currentSession) {
            const duration = Math.floor((new Date().getTime() - currentSession.start.getTime()) / 1000);
        ...
        if (sessionsList) {
            ...
            sessionsHistory.forEach(session => {
                const duration = Math.floor((session.end.getTime() - session.start.getTime()) / 1000);
